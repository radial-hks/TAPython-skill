# TAPython Tool Hub

[中文说明](README_CN.md)

TAPython Tool Hub is a standalone sharing site for TAPython/Chameleon editor tools. It is designed to collect, search, distribute, and compare installable editor tool packages. The site shares tools, not Skills.

The Phase 1 implementation uses a static-site workflow. Tool records are maintained as local JSON files, and the build step generates static API files and downloadable manifests. The output can be hosted on an internal company nginx server or any static file server.

## Current Features

- Tool catalog home page with tool count, review status, version snapshots, and deployment mode.
- Search and filters by tool name, tags, author, Unreal API, widget Aka, category, risk level, and review status.
- Tool detail page with feature summary, compatibility, install path, mount point, dependencies, risk notes, and install steps.
- Manifest view with a structured table and raw JSON view for future Agent-based installation.
- Version comparison for manifest field diffs and file list/hash diffs.
- Submission guide describing the required documentation, Chameleon UI JSON, Python controller, MenuConfig snippet, manifest, and version metadata.
- Static API generated at `/api/tools/index.json` and `/api/tools/<tool>.json`.

## Tech Stack

- Frontend: Vite + React + TypeScript
- UI: Ant Design
- Data source: `data/tools/*.json`
- Static generation: Node.js script
- Deployment: static file hosting, with company LAN nginx as the primary target

## Quick Start

```bash
npm install
npm run dev -- --host 127.0.0.1
```

Default development URL:

```text
http://127.0.0.1:5174/
```

Production build:

```bash
npm run build
```

The build runs `npm run generate:data` first, generates static API files and downloadable manifests, then writes the production bundle to `dist/`.

## Directory Structure

```text
tool-hub-site/
├── data/
│   └── tools/                  # Source tool data, one JSON file per tool
├── docs/                       # Implementation notes, API contracts, and follow-up notes
├── public/
│   ├── api/tools/              # Static tool API generated by the build script
│   └── downloads/              # Generated manifests and download README files
├── scripts/
│   └── generate-data.mjs       # Generates public/api and downloads from data/tools
├── src/
│   ├── App.tsx                 # Main UI, detail view, diff view, and submission guide
│   ├── data/registry.ts        # Frontend local registry
│   ├── main.tsx                # React entry and Ant Design setup
│   ├── styles.css              # Site styles
│   └── types.ts                # Tool, version, manifest, and diff type definitions
├── tapython-tool-hub-site.md   # Full execution plan
└── vite.config.ts              # Vite configuration
```

## Data Flow

1. Maintain tool metadata and version records under `data/tools/`.
2. Run `npm run generate:data`.
3. The script generates:
   - `public/api/tools/index.json`
   - `public/api/tools/<tool-slug>.json`
   - `public/downloads/<tool-slug>/<version>/manifest.json`
   - `public/downloads/<tool-slug>/<version>/README.md`
4. The frontend reads the local registry to render the catalog and links to generated manifests.

## Tool Data Requirements

Each tool should describe at least:

- Basic metadata: `slug`, name, description, author, category, tags, risk level, and review status.
- Compatibility: Unreal Engine versions, TAPython version, and required plugins.
- Installation: install path, Chameleon mount point, and MenuConfig snippet.
- Versions: version number, release date, changelog, manifest, file list, and hashes.
- Risk notes: modified objects, pre-checks, post-checks, and rollback guidance.

The current sample tool is `Actor Rename Tool`, defined in `data/tools/actor-rename-tool.json`.

## Deployment

The Phase 1 site can be deployed as static files:

```bash
npm run build
```

Publish `dist/` to nginx, an internal file server, or a cloud static hosting service. The generated API files and downloadable manifests are included in the static output, so no backend service is required for Phase 1.

If real uploads, review workflows, permissions, version archives, and download statistics are needed later, the current manifest-first model can be extended with FastAPI, PostgreSQL, and object storage.

## Related Documents

- [tapython-tool-hub-site.md](tapython-tool-hub-site.md): Full site plan and phase roadmap.
- [docs/implementation-notes.md](docs/implementation-notes.md): Phase 1 implementation notes and API details.
